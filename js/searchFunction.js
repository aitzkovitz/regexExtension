...
// global categories
globalCategories = [
	'disclaimers', 'phone', 'full-date', 'day', 'month', 'year', 'state', 'zip', 'email', 'price1', 'percent', 'time', 'address'
];

chrome.runtime.onMessage.addListener( function(request, sender, sendResponse){
	"use strict";
	if (request.action === "updateView"){
		var newSelections = request.updateView;
		// remove anything that isn't checked

		let difference = globalCategories.filter(x => newSelections.indexOf(x) == -1);
		console.log(difference);
		for (var i = 0; i < difference.length; i++){
			$('.' + difference[i]).removeClass('highlighted');			
		}

		// add highlights for new checks
		for (var i = 0; i < newSelections.length; i++){
			$('.' + newSelections[i]).not('.highlighted').addClass('highlighted');
		}
	...
	if (request.action === "initialSearch"){
		// function to find and replace matching textNodes
		function replaceNodes(node, regexToMatch, category) {
			...
			function replaceSingleNode(newNode, regexToMatch){
				...
				newSpan.className = 'foundMatch ' + category;
			...
			if (stringToMatch.match(regexToMatch) === null ){
				...
				var remainder1 = replaceSingleNode(node,regexToMatch);
				extraNodes.push(remainder1);
			} else {
				// save the node on the right that was created because of the split
		...
		if(!manualMode){
			...
			while(typeof(textNodes[i]) !== 'undefined'){
				...
				for (j = 0; j < formArray.length; j++){
					...
					leftOvers = replaceNodes(textNodes[i],regexObj[formArray[j]], formArray[j] );
			...
			while(i < textNodes.length){
				replaceNodes(textNodes[i], regexMatch, 'custom');
				i++;
			}
		}
